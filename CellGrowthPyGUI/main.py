...
import sys
sys.path.insert(1, "/Users/dspaande/Documents/GitProjects/Broad_DJ_AI/CellGrowthPyGUI/Classes/")
...
# Currently use sys to get other script
sys.path.insert(1, "/Users/dspaande/Documents/GitProjects/Broad_DJ_AI/CellGrowthPyGUI/Utils/")
...
def main():
    ...
    #display_message()
    predict(Cache_json=Cache_json, Cache_pickle=Cache_pickle, TumorType=args.TumorType, Tissue=args.Tissue, Dimension=args.Dimension, maf=args.maf, cnv=args.cnv, media=False, supplements=False)
...
def predict(Cache_json, Cache_pickle, TumorType, Tissue, Dimension, maf, cnv, media=False, supplements=False):
    ...
    print(len(one_hot_features.levels))
    #TumorType info
    ...
    for i in range(0, length_loop):
        for j in range(0, length_loop):
            ...
            if i == j:
                media_tmp[i] = 1
                media_name = media.levels[i]
            else:
                media_tmp[i] = 1
                media_tmp[j] = 1
                media_name = f"{media.levels[i]} 50% and media_tmp{media.levels[j]} 50%"
