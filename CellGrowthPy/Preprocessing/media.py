...
## SHOULD BE IN PREPROCESSING TERRA
...
def media(data, Path=False, Save=False, Encoding="Extract", Show=True):
    '''
    ...
    # Remove only null types media
    data = data[data["Media_type"] != "null:50%"]
...
if __name__ == '__main__':
    ...
    parser.add_argument("-e", dest="Encoding", nargs='?', default='One-hot', help="which representation of media you would like", choices=["Extract", "One-hot", "One-hot-percentage"])

    args = parser.parse_args()
    start = time.time()
    data = media(data=False, Path=args.Path, Save=args.Save, Encoding=args.Encoding, Show=str_to_bool(args.Show))
    end = time.time()
